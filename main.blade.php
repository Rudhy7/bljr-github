...
* CONTACT ME ON : 082240767189
-->
<html lang="en">
<meta http-equiv="content-type" content="text/html;charset=UTF-8" />

<head>
    <title>{{ !$config ? '' : $config->web_title }} - {{ !$config ? '' : $config->web_description }}</title>
    <meta charset="UTF-8">
    ...
    <meta property="og:title" content="{{ !$config ? '' : $config->web_title }} - {{ !$config ? '' : $config->web_keywords }}">
    <meta property="og:description" content="{{ !$config ? '' : $config->web_title }} - {{ !$config ? '' : $config->web_description }}">
    <meta name="robots" content="index, follow">
    <meta content="desktop" name="device">
    <meta name="author" content="{{ !$config ? '' : $config->web_title }}">
    <meta name="coverage" content="Worldwide">
    <meta name="apple-mobile-web-app-title" content="{{ !$config ? '' : $config->web_title }}">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-touch-fullscreen" content="yes">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <meta name="twitter:image" content="{{ !$config ? '' : $config->fav_ico }}" />
    <meta property="og:image" content="{{ !$config ? '' : $config->fav_ico }}">
...
.btn-primary {
    color: #fff;
    background-color: rgb(184, 15, 15) !important;
    border-color: transparent;
}
...
<!-- End Google Tag Manager (noscript) -->
    ...
    <div class="fixed bottom-0 right-4 z-40 flex transform flex-col rounded-t-lg ring border-t-0 p-0.5 !pb-0 text-text-color-foreground transition-all">
    <a href="{{ !$config ? '' : $config->wa_admin }}" class="fixed-btn chat-cs-btn bg-red-600 rounded-t-lg p-3 hover:bg-gray-900 flex items-center space-x-2.5" target="_blank" rel="noopener noreferrer" style="outline: none;">
    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 stroke-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
    ...
    <span class="hidden text-sm font-medium md:inline-flex">Chat CS</span>
  </a>
  </div>
